        ...
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
              <Calendar size={10} />
              {showYearAvg ? (
                <span className="text-blue-600 font-bold bg-blue-50 px-1.5 py-0.5 rounded">{selectedYear} YILLIK ORTALAMA</span>
              ) : (
                <span>{selectedYear} Dönemi - {MONTH_NAMES[selectedMonth]}</span>
              )}
            </div>
          </div>
          ...
          {!showYearAvg && (
            <>
              ...
              {MONTH_NAMES.map((m, i) => {
                if (i === 0) return null;
                return (
                  <button key={i} onClick={() => setSelectedMonth(i)} className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-all snap-center ${i === selectedMonth ? "bg-slate-800 text-white shadow-md" : "bg-white border border-slate-200 text-slate-500"}`}>
                    {m}
                  </button>
                );
              })}
      ...
      <div className="p-4 space-y-4">
        {displayData ? (
          <>
            <div className={`rounded-2xl shadow-lg mb-4 relative overflow-hidden flex flex-col text-center ${isTeslimBasarisiz ? "bg-gradient-to-br from-red-600 to-rose-700 shadow-red-200 text-white" : "bg-gradient-to-br from-emerald-500 to-green-700 shadow-emerald-200 text-white"}`}>
              ...
              {displayRegionData && (
                <div className="bg-black/10 py-2 flex items-center justify-center gap-2 border-t border-white/10">
                  ...
                  <span className="text-sm font-bold">{formatNumber(displayRegionData.teslimPerformansi)}%</span>
                </div>
              )}
            </div>

            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 pl-1">{showYearAvg ? "Yıllık Operasyonel Ort." : "Operasyonel"}</h3>
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 pl-1">{showYearAvg ? "Yıllık Dijital Ort." : "Dijital"}</h3>
              <div className="grid grid-cols-3 gap-2">
            ...
            <div>
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex justify-between items-center gap-4">
                <div className="flex-1 text-center border-r border-slate-100 flex flex-col items-center">
                  ...
                  <p className="text-2xl font-bold text-slate-800">{formatNumber(displayData.gelenKargo)}</p>
                </div>
                <div className="flex-1 text-center flex flex-col items-center">
                  <Box size={24} className="text-slate-300 mb-2" />
                  <p className="text-xs text-slate-400 mb-1 font-bold uppercase">Giden</p>
                  <p className="text-2xl font-bold text-slate-800">{formatNumber(displayData.gidenKargo)}</p>
                </div>
              </div>
            </div>
          </>
