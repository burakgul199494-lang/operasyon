  ...
  const generatePDF = async () => {
    ...
    try {
        ...
        const capturePage = async (elementId, pageNum) => {
            ...
            const canvas = await html2canvas(element, { scale: 2, useCORS: true, logging: false, windowWidth: 1600 });
            ...
            const marginX = 8; const marginTop = 25; const marginBottom = 8;
            ...
            if (finalImgHeight > maxImgHeight) { finalImgHeight = maxImgHeight; finalImgWidth = maxImgHeight * canvasRatio; }
            
            const xOffset = marginX + (maxImgWidth - finalImgWidth) / 2;
            
            pdf.setFont("Roboto", "bold"); pdf.setFontSize(16); pdf.setTextColor(30, 58, 138); 
            pdf.text(`TREND ANALİZ RAPORU: ${selectedUnit} - Sayfa ${pageNum}`, 14, 12);
            pdf.setFont("Roboto", "normal"); pdf.setFontSize(10); pdf.setTextColor(100);
        ...
        console.error(error); alert("PDF oluşturulamadı.");
    } finally { 
        if (desktopWrapper) { desktopWrapper.classList.remove('block'); desktopWrapper.classList.add('hidden', 'md:block'); }
        setIsGeneratingPdf(false); 
    }
  };

  const hasAnyData = (dataArray) => dataArray.some(d => d.current !== null || d.previous !== null);
  const sortedUnits = useMemo(() => [...UNITS].sort((a, b) => (a === "BÖLGE" ? -1 : (b === "BÖLGE" ? 1 : a.localeCompare(b, 'tr-TR')))), []);

  const renderCard = (metric) => {
      const trendStatus = getTrendStatus(trendData[metric.key], metric.key);

      return (
          // ÇÖZÜM: flex-col ve min-h gibi esnek kodlar tamamen silindi.
          // Sadece block bir yapı kullanılarak çökme %100 engellendi.
          <div 
              key={metric.key} 
              onClick={() => setExpandedMetric(metric)}
              className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-4 sm:p-5 relative transition-all hover:-translate-y-1 hover:shadow-xl cursor-pointer group block"
          >
              ...
              <div className="flex justify-between items-start mb-4 pl-2 pr-6">
                  <div className="min-w-0 flex-1">
                     <h3 className="font-bold text-slate-800 dark:text-white text-sm sm:text-base leading-tight break-words">{metric.label}</h3>
                  ...
                  <div className="bg-slate-900/90 backdrop-blur-sm px-2 py-1 rounded-lg text-[9px] sm:text-[10px] font-bold text-white shadow-md border border-white/10 text-center">
                     Hedef<br/><span style={{ color: '#fff' }}>{metric.target !== null ? `${metric.target}${metric.isPercent ? "%" : " Adet"}` : "-"}</span>
                  </div>
              </div>
              
              {/* ÇÖZÜM: Grafiğin yüksekliği sabitlendi. Asla kaybolamaz veya 0 piksel olamaz. */}
              ...
              {/* Eğilim Yazısı Her Zaman Kartın Altında Görünür */}
              {trendStatus && (
      ...
      <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-50 shadow-sm border-b border-slate-200 dark:border-slate-800 shrink-0 sticky top-0">
          <div className="px-4 py-3 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2"><div className="bg-blue-100 dark:bg-blue-900/30 p-2 rounded-lg"><TrendingUp className="text-blue-600 dark:text-blue-400" size={20} /></div><h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white tracking-tight hidden sm:block">Trend Analizi</h1></div>
              </div>
              <div className="flex items-center gap-2">
                  <button onClick={() => setIsComparisonMode(!isComparisonMode)} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-xs shadow-md transition-all ${isComparisonMode ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>{isComparisonMode ? <EyeOff size={16}/> : <Eye size={16}/>}<span className="hidden sm:inline">{isComparisonMode ? "Kıyaslamayı Kapat" : "Geçen Yılı Ekle"}</span></button>
                  <button onClick={generatePDF} disabled={isGeneratingPdf} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold text-xs shadow-md transition-colors disabled:opacity-50">{isGeneratingPdf ? <Loader2 size={16} className="animate-spin" /> : <FileDown size={16} />}<span className="hidden sm:inline">PDF Aktar</span></button>
              </div>
          </div>
          <div className="px-4 pb-3 flex flex-wrap gap-3 items-center mt-2 sm:mt-0">
              <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg px-3 border border-slate-200 dark:border-slate-700"><Calendar size={14} className="text-slate-500 dark:text-slate-400 mr-2" /><select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-transparent text-slate-800 dark:text-slate-200 font-bold text-sm py-1.5 border-none outline-none focus:ring-0">{availableYears.map((y) => <option key={y} value={y}>{y}</option>)}</select></div>
              <div className="relative flex-1 max-w-[300px]"><select value={selectedUnit} onChange={(e) => setSelectedUnit(e.target.value)} className="appearance-none w-full bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 font-bold text-sm py-1.5 pl-3 pr-8 rounded-lg border border-blue-200 dark:border-blue-800 outline-none focus:ring-2 focus:ring-blue-500 transition-colors">{sortedUnits.map((u) => <option key={u} value={u}>{u}</option>)}</select><ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-blue-600 pointer-events-none" /></div>
          </div>
      </div>
      
      <div className="p-4 sm:p-6 max-w-[1800px] mx-auto w-full">
          <div className="flex flex-col gap-4 md:hidden">{METRICS.map(renderCard)}</div>
          <div id="desktop-wrapper" className="hidden md:block">
              <div id="pdf-page-1" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-4 sm:mb-6">{METRICS.slice(0, 8).map(renderCard)}</div>
              <div id="pdf-page-2" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">{METRICS.slice(8, 16).map(renderCard)}</div>
          </div>
      </div>

      {/* TIKLAYINCA AÇILAN DEV EKRAN MODÜLÜ */}
      {expandedMetric && (
          <div className="fixed inset-0 bg-slate-900/80 flex items-center justify-center z-[70] p-4 sm:p-8 backdrop-blur-sm" onClick={() => setExpandedMetric(null)}>
              <div className="bg-white dark:bg-slate-900 w-full max-w-6xl rounded-3xl shadow-2xl flex flex-col relative animate-in zoom-in duration-200 overflow-hidden border border-slate-200 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
                 <div className="p-5 sm:p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start gap-4 bg-slate-50/50 dark:bg-slate-800/50">
                    <div>
                       <h2 className="text-xl sm:text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3"><div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full" style={{ backgroundColor: expandedMetric.color }}></div>{expandedMetric.label}</h2>
                       <div className="flex gap-4 mt-3 sm:mt-4"><div className="flex items-center gap-2 text-sm sm:text-base text-slate-700 dark:text-slate-300 font-bold"><div className="w-3 h-3 rounded-full" style={{backgroundColor: expandedMetric.color}}></div> {selectedYear}</div>{isComparisonMode && (<div className="flex items-center gap-2 text-sm sm:text-base text-slate-500 font-bold"><div className="w-3 h-3 rounded-full border-2 border-slate-400 border-dashed bg-transparent"></div> {selectedYear - 1}</div>)}</div>
                    </div>
                    <div className="flex items-start gap-3 sm:gap-4"><div className="bg-slate-900/90 backdrop-blur-sm px-4 py-2 sm:py-3 rounded-xl text-sm sm:text-base font-bold text-white shadow-md border border-white/10 text-center">Hedef<br/><span style={{ color: '#fff' }}>{expandedMetric.target !== null ? `${expandedMetric.target}${expandedMetric.isPercent ? "%" : " Adet"}` : "-"}</span></div><button onClick={() => setExpandedMetric(null)} className="p-2 sm:p-3 bg-slate-200 dark:bg-slate-800 text-slate-500 hover:text-slate-800 dark:hover:text-white rounded-xl transition-colors"><X size={24} /></button></div>
                 </div>
